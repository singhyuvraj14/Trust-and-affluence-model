# Generated by Django 3.2.7 on 2021-09-29 18:11

from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='User',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('loanoriginalamount', models.IntegerField()),
                ('statedmonthlyincome', models.IntegerField()),
                ('term', models.IntegerField()),
                ('employmentstatus', models.IntegerField()),
                ('monthlyloanpayment', models.FloatField(null=True)),
                ('debttoincomeratio', models.FloatField(null=True)),
                ('isborrowerhomeowner', models.BooleanField(default=True)),
                ('socialmediaaccess', models.IntegerField()),
                ('friends', models.IntegerField()),
                ('postbyuser', models.IntegerField()),
            ],
        ),
    ]
